...
func (h *Handler) get(c *gin.Context) {
	const op = "implementors.Handler.get"
	...
	resp, err := h.service.Get(c.Request.Context(), &implementors.GetImplementorRequest{Id: int32(id)})
	if err != nil {
		log.Error("Failed to get implementor", slog.Any("error", err))
		c.JSON(http.StatusInternalServerError, respond.ErrorResponse("internal server error"))
		return
	}

	log.Info("Implementor retrieved successfully", slog.Any("response", resp))
