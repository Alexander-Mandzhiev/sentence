...
func (r *Repository) Create(ctx context.Context, sentence *sentences.SentenceResponse) error {
	...
	if sentence == nil || sentence.StatusId <= 0 || sentence.Name == "" || sentence.DirectionId <= 0 || sentence.PriorityId <= 0 {
		r.logger.Error("Invalid input data", slog.String("op", op), slog.Any("sentence", sentence))
	...
	err := r.db.QueryRow(ctx, query, sentence.StatusId, sentence.Name, sentence.Phone, sentence.DepartmentId, sentence.Problem, sentence.Solution, sentence.DirectionId, sentence.ImplementorId, sentence.PriorityId, sentence.Encouragement).Scan(&id, &createdAt)
	if err != nil {
	...
	r.logger.Info("Sentence created", slog.String("op", op), slog.Any("sentence", sentence))
	return nil
}
