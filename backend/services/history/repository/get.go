...
func (r *Repository) Get(ctx context.Context, req *history.GetHistoryRequest) (*history.HistoryResponse, error) {
	...
	query := `SELECT id, sentence_id, status_id, created_at, reason, details FROM histories	WHERE id = $1`
	...
	if err != nil {
		...
		r.logger.Error("failed to get history", slog.String("op", op), slog.String("error", err.Error()))
